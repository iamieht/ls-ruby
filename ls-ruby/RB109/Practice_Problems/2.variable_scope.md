...
This code raises a `NameError` exception when invoking the `puts` method with `a` as an argument.

On line 1, the local variable `array` is initialized and references an Array Object with 3 Integer Objects `[1, 2, 3]`.

On line 3, the `each` method is called on the object referenced by `array` and gets passed a `do..end` block as an argument. This method iterates over each element in the array, passing each element to the block in turn.

Inside the block, the local variable `a` is initialized and references the object referenced by the block parameter `element`. During each iteration, `a` reference the element of the array in turn. After the block finishes executing, `a` will have the value of the last element of the array (3).

On line 7, we call the `puts` method with `a` as an argument, raising a `NameError` exception, as the variable `a` cannot be referenced in the outer scope, as it was initialized inside the block.

This code demonstrates the concept of local variable scope in relation to blocks after a method invocation, in which blocks create a new inner scope and the variables initialized in the inner scope cannot be referenced in an outer scope.
...
On line 1, the local variable `a` is initialized and references an Integer Object with value `7`.

On line 2, the local variable `array` is initialized and references an Array Object with 3 Integer Objects `[ 1, 2, 3]`.

On line 4, the `each` method is called on the. object referenced by `array` and gets passed a `do..end` block as an argument.

As the block parameter `a` has the same name as the local variable `a` initialized in the outer scope, the block parameter shadows the `a` variable in the outer scope. Inside the block, the block local variable `a` is reassigned to the returned value of invoking the `+` method on the value referenced by `a` with `1` as an argument. After the block finishes executing, `a` will have the value `4`.

On line 8, we call the `puts` method with `a` as an argument, outputting `7`, as the local variable `a` is not affected by the reassignment within the block.

This code snippet demonstrates the concept of variable shadowing and local variable scope in blocks after a method invocation.
...
On line 10, we call the method `my_value` with `array` as an argument. The value referenced by `array` is bound to the method parameter `ary`.

Inside the method, we call the `each` method on the object referenced by `ary` and gets passed a `do..end` block as an argument. The `each` method iterates over each element in the array, passing each element to the block in turn.

On line 6, we invoke the `+` method on the value referenced by `a` with the value referenced by `b` as an argument. This raises an exception as the variable `a` hasn't been initialized at this point and cannot be referenced.

On line 11, the `puts` method would have been invoked with `a` as an argument, if the code execution would have reached to this point, but since an exception was raised during the `my_value` method invocation, the code execution stop at that point.

This code snippet demonstrates the concept of variable scope in relation to a method definition and in relation to a block after a method invocation. Variables cannot be referenced if they are not initialized first.
...
On line 1, the local variable `x` is initialized and references an Integer object with value `10`.

On lines 2, we call the `times` method on `5` and it gets passed a `do..end` block as an argument. As the block parameter `x` has the same name as the local variable `x` initialized in the outer scope, it shadows `x` in the outer scope, preventing access to it inside the block. Instead, the `times` method pass the values `0`, `1`, `2`, `3` and `4` to the block in turn, which are then outputted on separate lines when we call the `puts` method with `x` as an argument.
...
On line 2, we call the `times` method on `3` and gets passed a `do..end` block as an argument. The `times` method yields during each iteration a value from `0` to `2` that gets assigned to the block parameter `n`. 

Within the block, `a` is reassigned to `3` and `b` is initialized and references the value `5`.

On line 6, the method `puts` is called with `a` as an argument, outputting `3`

On line 7, the method `puts`is called with `b`as an argument, raising an exception as there is not variable `b` available in the outer scope, as it was initialized inside the block, which creates an inner scope after a method invocation.

This code snippet demonstrates the concept of variable scope in relation to blocks after a method invocation, as well as reassignment.
